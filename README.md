...
在"升级"前文星宿舍区已至少进行了1年的p2p流量特征检测，已知封禁策略与宿舍的物理位置和运营商都有关(疑似部分楼层不限速，可能是忘了设置)，**建议对该类流量进行代理或做转发**
...
在"升级"前文星宿舍区已至少进行了1年的p2p流量特征检测，会劫持DNS响应。"升级"后似乎未启用该检测
...
深度包检测的开销较大，按照惯例，目标ip归属地是重要检测依据，为提升检测效率，该系统可能直接放行目的IP为内地的流量(因为代理的终点一半都在海外)
